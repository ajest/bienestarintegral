<template>
	<div class="row">		
		...
		<popupdeleteconfirm v-on:success="operationSuccess" v-on:error="operationError" v-bind:element_id="series_id" v-bind:elements="series" v-bind:url="url" v-bind:delete_text_confirm="delete_text_confirm"></popupdeleteconfirm>
	</div>
</template>
<script>
	...
	export default {
		...
		methods: {
			paginationCallback(){
				...
				axios.get('/series/list/' + (t.$route.params.id ? t.$route.params.id : 1) + '/' + (t.$route.params.field ? t.$route.params.field : ''), 
					{
						params: {
			...
			searchSeries(){
			    ...
			    t.delayTimer = setTimeout(function() {
			    	...
			    	if(t.search_in_table){
			    		...
			    		axios.get('/series/search', {params: {'filtro_status': t.filtros.status, 'term': (t.search_in_table ? t.search_in_table : '')}})
							.then(function (response) {
								t.series = [];
