	...
	export default {
		...
		methods: {
			paginationCallback(){
				...
				axios.get('/specialties/list/' + (t.$route.params.id ? t.$route.params.id : 1) + '/' + (t.$route.params.field ? t.$route.params.field : ''), 
					{
						params: {
			...
			searchSpecialties(){
			    ...
			    t.delayTimer = setTimeout(function() {
			    	...
			    	if(t.search_in_table){
			    		...
			    		axios.get('/specialties/search', {params: {'filtro_status': t.filtros.status, 'term': (t.search_in_table ? t.search_in_table : '')}})
							.then(function (response) {
								t.specialties = [];
